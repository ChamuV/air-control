# src/aircontrol/plugins/detectors/thumbs_down_hold.py
...
class ThumbsDownHoldDetector:
    """
    Continuous hold detector for gesture.volume_down_hold.
    Emits repeatedly while a stable thumbs-down pose is held.
    """

    def __init__(
        self,
        hold_frames: int = 3,
        repeat_every_frames: int = 4,
        move_tol: float = 0.10,
        min_detect_frames: int = 2,
        thumb_down_y_margin: float = 0.07,
        thumb_side_limit: float = 0.10,
    ...
    def update(self, hand_landmarks) -> list[GestureEvent]:
        ...
        if not self._is_thumbs_down(lms):
            self._reset()
            return []
        ...
        if self._hold_counter >= self.hold_frames:
            ...
            if offset % self.repeat_every_frames == 0:
                return [GestureEvent("gesture.volume_down_hold", {})]

        return []


class ThumbsDownHoldPlugin:
    def register(self, ctx: AppContext) -> PluginRegistration:
        return PluginRegistration(detectors=[ThumbsDownHoldDetector()], actions={})


def plugin():
    return ThumbsDownHoldPlugin()