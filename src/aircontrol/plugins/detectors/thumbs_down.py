# src/aircontrol/plugins/detectors/thumbs_down.py
...
class ThumbsDownDetector:
    ...
    def _finger_folded(self, lms, tip: int, mcp: int) -> bool:
        # Folded if fingertip is below its MCP in image coordinates (y increases downward)
        return lms[tip].y >= lms[mcp].y

    def _thumb_down(self, lms) -> bool:
        return (lms[THUMB_TIP].y > lms[WRIST].y) and (lms[THUMB_TIP].y > lms[THUMB_MCP].y)
    ...
    def update(self, hand_landmarks) -> list[GestureEvent]:
        ...
        gesture = self._thumb_down(lms) and idx_fold and mid_fold and rng_fold and pky_fold
        ...
        if self._hold_counter >= self.hold_frames:
            ...
            self._hold_counter = 0
            self._detect_streak = 0
            return [GestureEvent("gesture.thumbs_down", {})]

        return []


class ThumbsDownPlugin:
    """Detector-only plugin: emits gesture.thumbs_down."""
    def register(self, ctx: AppContext) -> PluginRegistration:
        detector = ThumbsDownDetector()
        return PluginRegistration(detectors=[detector], actions={})


def plugin():
    return ThumbsDownPlugin()